...
- **Firebolt / Fireball** (`Game/FX/Firebolt.cs/.gdshader`): Ember sphere, turbulent tail and burning impact; shared `SpellProjectile` lifecycle, 0.18-second cosmetic flight and canonical `firebolt.tres` preset. See [[spell-effect-system]].
- **Magic Arrow** (`Game/FX/MagicArrow.cs/.gdshader`): Arcana lance, release seal and crystalline impact, attached to casting palm/target bounds and scaled to the race. Cosmetic 0.14-second flight for the server’s zero-travel spell; reflection and dodge handled separately. Runtime and preview: [[spell-effect-system]].
