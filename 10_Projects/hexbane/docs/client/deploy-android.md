...
### Follow-up: user-exported v15 omitted PGS

The user confirmed that their own `android/hexbane_play.aab` version 15 was uploaded and installed through Play. Inspection of that artifact found neither PGS manifest metadata nor PlayGamesSdk/native bridge DEX classes. The current preset had an empty Game ID despite the earlier separately verified export. Export exclusion filters had also reverted. An open editor retaining old configuration is a plausible cause; this was not directly proven.

After the user closed Godot, restored Game ID `497120553759` in Android Play, incremented versionCode to 16, and restored OAuth JSON/keystore exclusions across presets. The local Android preset remains opted out with an empty ID. Close the editor before external changes to export_presets.cfg, then reopen it to load the saved settings. Future Android Play exports should retain this Game ID; increment versionCode for each uploaded release.
...
The login panel now hides email/password, registration, and server selection in release builds. Android keeps automatic Play Games followed by its retry button; iOS currently skips Game Center and exposes the Google browser fallback directly. Game Center and Sign in with Apple remain future providers.
