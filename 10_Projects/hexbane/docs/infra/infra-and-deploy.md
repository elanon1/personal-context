...
- **`apps-root`** (`gitops/argo/root-app.yaml`, app-of-apps over `gitops/argo/apps/`) has `automated: {}` since 2026-09-17 (no prune, no selfHeal): a committed change to an Application manifest reaches the cluster on its own. Before that date every `image.tag` bump needed a manual `kubectl apply` of the manifest. `root-app.yaml` itself is still applied by hand.
- Application `hexbane` has `automated.selfHeal: true` (2026-09-17, still no prune). Required: Argo CD 2.14 logs `Skipping auto-sync: selfHeal disabled and sync caused by object update` when the OutOfSync comes from a changed `helm.parameters` value in the Application object (which is exactly what a CI deploy does) rather than from a new commit in the chart repo. Measured end-to-end: GitOps push → new pod in ~3 min (git poll interval), no manual refresh.
...
## Local missing queue RPC repair (2026-09-12)

User reported `RPC function not found` on Vs player after production rollout. Current local Docker logs showed the game authentication/character request at17:08UTC; the local container still had migration5 and no queue_config registration. Production had queue_config and the new image. Cause: the client was using the old local backend, not the deployed production backend.

Added Compose passthrough for the three HEXBANE queue env variables (defaults false/false/global) and migration copying to `make dev`. Created gitignored local `.env` enabling queue/fallback; no production config changed. Saved a local pg_dump before upgrade, then `make dev` rebuilt/copied plugin+migrations and restarted. Local migration8 clean,42accounts/7characters preserved. Authenticated WebSocket queue_config returned enabled=true on127.0.0.1:7350; temporary probe account removed. Compose config validation, plugin build and diff check passed. A new physical-device UI click was not observed.
...
### Google Play Games credential activation (2026-09-16)

The production deployment initially loaded `GOOGLE_CREDENTIALS_JSON` but Nakama rejected the Web OAuth JSON because it had no `redirect_uris` array. The Android Play Games server exchange uses an empty redirect URI; the credential copy in Kubernetes was therefore normalized with `web.redirect_uris: [""]`, without exposing the secret. Nakama was restarted and logs now show `Startup done`; the pod is ready with zero restarts and Argo remains Synced/Healthy. Public HTTPS healthcheck returned 200. The secret remains `hexbane-google-credentials`; the source download in `Downloads` intentionally remains untouched.
