...
- **2026-09-17 — Auto-deploy serwera: job `deploy` w CI commituje `image.tag` do `elanon1/argocd`, plus bramka `migrate-check` na świeżym Postgresie.** **Why:** Argo CD ma już automated sync, więc jedyne brakujące ogniwo to commit w GitOps; commit z CI zostawia historię deployów i prosty rollback (revert), bez instalowania Argo Image Updatera i jego poll-owania GHCR. Migracje i tak wykonują initContainery przy starcie poda, więc bramka w CI ma je zepsuć w pipeline, nie w klastrze. Deploy key z zapisem zamiast PAT-a: zakres ograniczony do jednego repo. `apps-root` dostał `automated: {}` (bez prune/selfHeal), bo bez tego commit CI w manifeście Application nie trafiał do klastra; alternatywę (tag w osobnym pliku values jako `$values`) odrzucono na życzenie Filipa na rzecz auto-syncu.
...
- **Why:** a fixed latest20 feed cached as JSON for60s coalesces concurrent reads and bounds memory/DB work without a new service. Other nodes converge by TTL; errors retry after5s. Client integration now uses the shared feed cache and stable selection IDs. Migration10 seeds one English welcome announcement; old client fixtures are removed. See [[news]].
...
- **Why:** User chose both values for simultaneous damage 10 / healing 3; net HP change hides what happened. Event-driven presentation retains that information without changing combat or inventing hits from reconnect snapshots.
